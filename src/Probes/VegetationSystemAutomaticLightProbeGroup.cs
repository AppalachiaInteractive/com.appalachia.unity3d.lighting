    ...
    public class VegetationSystemAutomaticLightProbeGroup : AutomaticLightProbeGroup
    {
        ...
        protected override void GenerateProbesForTargets(AppaList<Vector3> points, ref bool canceled)
        {
            ...
            for (var cellIndex = 0; cellIndex < system.VegetationCellList.Count; cellIndex++)
            {
                ...
                for (var packageIndex = 0; packageIndex < cell.VegetationPackageInstancesList.Count; packageIndex++)
                {
                    var packageInstance = cell.VegetationPackageInstancesList[packageIndex];
                    var package = system.VegetationPackageProList[packageIndex];
                    
                    for (var itemIndex = 0; itemIndex < packageInstance.VegetationItemMatrixList.Count; itemIndex++)
                    {
                        var matrices = packageInstance.VegetationItemMatrixList[itemIndex];
                        var info = package.VegetationInfoList[itemIndex];

                        if (info.EnableExternalRendering) continue;
                        if (info.VegetationType == VegetationType.Grass) continue;
                        if (info.VegetationType == VegetationType.Objects) continue;
                        if (info.Bounds.size.magnitude < 1.0) continue;
