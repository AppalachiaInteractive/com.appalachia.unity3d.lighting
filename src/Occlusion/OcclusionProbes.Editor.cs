...
namespace Appalachia.Lighting.Occlusion
{
    public partial class OcclusionProbes : MonoBehaviour
    {
        ...
        [Header("Objects")]
        public GameObject occlusionBakeObjects;
        ...
        void Started()
        {
            ...
            {
                Debug.LogWarning("Lightmapping started.  Occlusion probes will not bake as Baked GI is not enabled.");
                return;
            }
            ...
            foreach(var detail in m_OcclusionProbesDetail)
            {
                ...
                var localToWorldDetail = Matrix4x4.TRS(t.position - sizeDetail * 0.5f, t.rotation, sizeDetail);
        ...
        {
            var minProbeDistance = Mathf.Min(Mathf.Min(size.x / (count.x - 1), size.y / (count.y - 1)), size.z / (count.z - 1));
        ...
        void Completed()
        {
            ...
            if (!Lightmapping.bakedGI)
            {
                Debug.LogWarning("Lightmapping completed.  Occlusion probes did not bake as Baked GI is not enabled.");
                return;
            }
            ...
            {
                var dataCountDetail = m_CountBakedDetail[i].x * m_CountBakedDetail[i].y * m_CountBakedDetail[i].z;
